...
const Rooms = () => {
  ...
  useEffect(() => {
    fetch("https://assignment-11-server-side-smoky.vercel.app/addroom")
      ...
      {room !== null ? (
        <div className="mt-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {room
              .filter((p) => {
                return search.toLowerCase() === "" ? p : p.price.toLowerCase().includes(search);
              })
              .map((roomInfo) => (
                <RoomCard key={roomInfo._id} roomInfo={roomInfo}></RoomCard>
              ))}
          </div>
        </div>
      ) : (
